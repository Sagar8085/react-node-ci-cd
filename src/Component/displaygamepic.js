...
export default function DisplayGamepic(props)
...
const[gamelistCategory,SetGameListCategory]=useState([]) 
...
const fillGamebySubcategoryid=async(sid)=>{
 var body={subcategoryid:sid}
  var result = await postData("game/displaygameintosubcategory",body)
  SetGameListCategory(result)
}
const FillGamecategory=()=>{
  return gamelistCategory.map((item)=>{
    return(
...
const handleDelete=async()=>{
    ...
    var result = await postData("gamepic/deletegamepic",body);
      if(result)
      {
        swal({
...
const handleClick=async()=>{
    ...
    if(isBlank(gameId))
    {error=true
    msg+="<font color='#eb4d4b'><b>Game should not be blank..</b></font><br>"
    } 
    msg+='</div>'
    if(error)
    {
   ...
   var result = await postData("gamepic/editgamepic",body);
    if(result)
    {
      swal({
...
const handleCancelImage=()=>{
  ...
  const handleClickSaveImage=async()=>{
    ...
    var result = await postDataAndImage("gamepic/editgameimage",formData,config);
    if(result)
    {
      swal({
...
return(
    ...
    <div className={classes.subdiv}>
        ...
        {FillSubcategory()}
            ...
            <Grid item xs={12}>
        <FormControl variant="outlined" className={classes.formControl} fullWidth>
        <InputLabel id="demo-simple-select-outlined-label-game">Game Id</InputLabel>
        <Select
          labelId="demo-simple-select-outlined-label-game"
          id="demo-simple-select-outlined-game"
          value={gameId}
       ...
       {FillGamecategory()}  
             ...
             <Grid item xs={6}>
            <span style={{fontSize:16,fontWeight:'300'}}>Upload Game image</span>
...
const FetchAllGamepic=async()=>{
    var result = await getData("gamepic/displaygamepic")
        setList(result)
    }
    useEffect(function(){
        FetchAllGamepic()
    },[])
    function displayAllGamepic() {
        return (
          <div>
          <MaterialTable
            title="Game List"

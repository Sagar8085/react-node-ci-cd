...
export default function DisplayAccessorypic(props)
...
const fillAccessorybySubcategoryid=async(sid)=>{
...
const handleDelete=async()=>{
    ...
    var result = await postData("accessorypic/deleteaccessorypic",body);
      if(result)
      {
        swal({
...
const handleClick=async()=>{
    ...
    {error=true
    msg+="<font color='#eb4d4b'><b>Accessories should not be blank..</b></font><br>"
    }
    msg+='</div>'
    if(error)
    {
   ...
   var result = await postData("accessorypic/editaccessorypic",body);
    if(result)
    {
      swal({
...
const handleCancelImage=()=>{
  ...
  const handleClickSaveImage=async()=>{
    ...
    var result = await postDataAndImage("accessorypic/editaccessoryimage",formData,config);
    if(result)
    {
      swal({
...
return(
    ...
    <div className={classes.subdiv}>
        ...
        {FillAccessorycategory()}
             ...
             <Grid item xs={6}>
            <span style={{fontSize:16,fontWeight:'300'}}>Upload Acccessories image</span>
...
const FetchAllAccessorypic=async()=>{
    var result = await getData("accessorypic/displayaccessorypic")
        setList(result)
    }
    useEffect(function(){
        FetchAllAccessorypic()
    },[])
    function displayAllAccessorypic() {
        return (
          <div>
          <MaterialTable
